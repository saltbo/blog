<template>
  <div class="footer">
    <div class="frow" v-if="$themeConfig.footer">
      {{copyright}} - <a class="beian" href="http://beian.miit.gov.cn/" target="_blank" v-if="icp">{{icp}}</a>
    </div>
    <div class="frow">
      本站已稳定运行：<span>{{runtime.days}}</span>天<span>{{runtime.hours}}</span>时<span>{{runtime.minutes}}</span>分<span>{{runtime.seconds}}</span>秒
    </div>
    <div class="frow">
      Powered by <a href="https://vuepress.vuejs.org/" target="_blank">Vuepress</a> & <a href="https://saltbo.cn/" target="_blank">Saltbo</a>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    icp() {
      return (
        (this.$themeConfig.footer && this.$themeConfig.footer.icp) || []
      )
    },
  },
  methods: {
